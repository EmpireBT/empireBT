(function () {
    ...
    EmpireBT.GameRunner = function GameRunner(game_data) {
        ...
        this.showMovesHandler = function (event_data, cb) {
            ...
            if (this.currentPlayer.currentTurnSp >= unit.sp && !unit.hasAttacked) {
                ...
                var subx, suby, pos1d, t, suby2, i;
                if (!unit.hasMoved && !unit.hasAttacked) {

                    for (i = 0; i <= unit.mov; i++) {
                        suby = y - i;
                        
                        //suby = 5 - 1
                        for (subx = x - unit.mov + i; subx < x + unit.mov - i; subx++) {
                            if (subx < 0 || subx >= EmpireBT.GameRunner.BATTLEFIELD_X) continue;
                            if (suby < 0 || suby >= EmpireBT.GameRunner.BATTLEFIELD_Y) continue;

                            pos1d = EmpireBT.GameRunner.posXyTo1D({ position : {x : subx, y: suby}});
                            t = this.tiles[pos1d];
                        ...
                        for (subx = x - unit.mov + i; subx < x + unit.mov - i; subx++) {
                            ...
                            pos1d = EmpireBT.GameRunner.posXyTo1D({ position : {x : subx, y: suby2}});
                            t = this.tiles[pos1d];
                            if (!t.isObstacle && t.unit === null) {
                                results.available_move_tiles.push(pos1d);
                            }
                ...
                if (!unit.hasAttacked) {
                    for (i = 0; i <= unit.range; i++) {
                        suby = y - i;
                        for (subx = x - unit.range + i; subx < x + unit.range - i; subx++) {
                            if (subx < 0 || subx >= EmpireBT.GameRunner.BATTLEFIELD_X) continue;
                            if (suby < 0 || suby >= EmpireBT.GameRunner.BATTLEFIELD_Y) continue;
    ...
    if (typeof module !== 'undefined' && module.exports) {
        ...
        window.GameRunner = EmpireBT.GameRunner;
    }
})();